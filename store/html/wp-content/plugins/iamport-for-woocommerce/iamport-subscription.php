			...
			if ( $initial_payment ) {
				...
				if ( $result->success ) {
					if ( $payment_data->status == 'paid' ) {
						...
						$message = sprintf( __( '정기결제 회차 과금(%s차결제)에 실패하였습니다. (status : %s)', 'iamport-for-woocommerce' ) , $order->suspension_count, $payment_data->status );
						$order->add_order_note( $message );
		...
		if ( $result->success ) {
			if ( $checking_amount > 0 ) {
				...
				if ( $checking_result->success && $checking_result->data->status == 'paid' ) {
					...
					$message = sprintf( __('카드등록 중 테스트 결제 %s원에 대한 결제가 실패하였습니다.(%s)', 'iamport-for-woocommerce'), number_format($checking_amount), $checking_result->data->fail_reason );
					$order->add_order_note( $message );
